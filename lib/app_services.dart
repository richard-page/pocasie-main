...
class CacheManager {
  ...
  static Future<void> saveWarnings(String cacheKey, String jsonBody) async {
    final prefs = await SharedPreferences.getInstance();
    final key = '$kWarningsCachePrefix$cacheKey';
    final data = {
      'timestamp': DateTime.now().millisecondsSinceEpoch,
      'body': jsonBody,
    };
  ...
  static Future<String?> getWarnings(String cacheKey) async {
    final prefs = await SharedPreferences.getInstance();
    final key = '$kWarningsCachePrefix$cacheKey';
    if (!prefs.containsKey(key)) return null;
    try {
      ...
      final timestamp = data['timestamp'] as int;
      final savedTime = DateTime.fromMillisecondsSinceEpoch(timestamp);
      if (DateTime.now().difference(savedTime).inMinutes > kWarningsCacheDurationMinutes) {
        return null;
      }
      return data['body'] as String;
    } catch (e) {
...
List<dynamic>? _decodeWarningsJsonList(String rawBody) {
  var s = rawBody.trim();
  ...
  try {
    ...
    if (decoded is Map) {
      ...
      if (m.containsKey('title') || m.containsKey('valid_to') || m.containsKey('valid_from')) {
        return <dynamic>[m];
      }
    }
  } catch (_) {}
...
List<WarningData> _warningsFromDecodedList(List<dynamic>? rawList) {
  if (rawList == null || rawList.isEmpty) return [];
  final out = <WarningData>[];
  for (final row in rawList) {
    final w = WarningData.fromJson(row is Map<String, dynamic> ? row : row);
    if (w.title.isEmpty && w.description.isEmpty && w.region.isEmpty) continue;
    out.add(w);
  }
  return out;
}

Future<List<WarningData>> fetchWarnings(GeoCity city) async {
  String countrySlug = 'slovakia';
  ...
  final cacheKey = '${countrySlug}_${city.admin1}_${city.admin2}_${city.name}'.replaceAll(' ', '_');

  Future<void> saveParsed(List<WarningData> list) async {
    await CacheManager.saveWarnings(cacheKey, json.encode(list.map((w) => w.toJson()).toList()));
  }

  /// Medzipamäť (15 min) ako záloha pri chybe siete.
  final cachedBody = await CacheManager.getWarnings(cacheKey);
  List<WarningData>? cachedParsed;
  if (cachedBody != null) {
    final cachedList = _decodeWarningsJsonList(cachedBody);
    if (cachedList != null) {
      cachedParsed = _warningsFromDecodedList(cachedList);
    }
  }

  String urlIndexPhp({required bool withGeo}) {
    final b = StringBuffer('$kWarningsBaseUrl/index.php?country=${Uri.encodeQueryComponent(countrySlug)}&format=json');
    if (withGeo) {
      b.write('&lat=${city.lat}&lon=${city.lon}');
    }
    if (city.admin1.isNotEmpty) b.write('&region=${Uri.encodeComponent(city.admin1)}');
    if (city.admin2.isNotEmpty) b.write('&county=${Uri.encodeComponent(city.admin2)}');
    if (city.name.isNotEmpty) b.write('&city=${Uri.encodeComponent(city.name)}');
    return b.toString();
  }

  String urlRoot({required bool withGeo}) {
    final b = StringBuffer('$kWarningsBaseUrl/?country=${Uri.encodeQueryComponent(countrySlug)}&format=json');
    if (withGeo) {
      b.write('&lat=${city.lat}&lon=${city.lon}');
    }
    if (city.admin1.isNotEmpty) b.write('&region=${Uri.encodeComponent(city.admin1)}');
  ...
  Future<List<WarningData>?> tryUrl(String urlStr) async {
    try {
      final response = await http
          .get(
            ...
            headers: const {
              ...
              'User-Agent': 'pocasie-app/1.0 (flutter; meteo-warnings)',
            },
          )
          .timeout(const Duration(seconds: 14));
      if (response.statusCode != 200 || response.body.isEmpty) return null;
      final list = _decodeWarningsJsonList(response.body);
      if (list == null) return null;
      return _warningsFromDecodedList(list);
    } catch (_) {
      return null;
    }
  }
  ...
  try {
    final candidates = <String>[
      buildHelkorWarningsUrl(city, countrySlug, formatJson: true),
      urlIndexPhp(withGeo: true),
      urlRoot(withGeo: true),
      urlIndexPhp(withGeo: false),
      urlRoot(withGeo: false),
    ];
    final seen = <String>{};
    for (final u in candidates) {
      if (!seen.add(u)) continue;
      final got = await tryUrl(u);
      if (got != null) {
        await saveParsed(got);
        return got;
      }
    }
  } catch (_) {}
