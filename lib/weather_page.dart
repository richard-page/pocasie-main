...
class _WeatherPageState extends State<WeatherPage> with WidgetsBindingObserver {
  ...
  bool isLoading = true, hasError = false, _isRefreshing = false;
  GeoCity? currentCity;
  ...
  bool _pendingOpenVystrahyFromWidget = false;
  bool _openingVystrahyFromWidget = false;
  ...
  Future<void> _applyRadarNowcastSignal({
    ...
    if (!mounted || mySerial != _weatherFetchSerial) return;
    if (radarSerial != _radarFetchSerial) return;
    // Nezmaž platný nowcast prázdnou odpoveďou — sledovač by zmizol pri výpadku API.
    if (!radarCtx.eligible && _radarNowcastContext.eligible) {
      return;
    }
  ...
  void _deferRadarSetup(GeoCity city, {bool forceReload = false}) {
    ...
    if (cityChanged && _radarController != null && mounted) {
      setState(() {
  ...
  void _markRadarContentReady() {
    ...
    _radarLoadTimeoutTimer = null;
    _applyRadarContentReady();
  }

  void _applyRadarContentReady() {
    _revealRadarMapLayers();
    if (!mounted) {
      _radarContentReady = true;
      _isRadarLoading = false;
      _radarMapUiReady = true;
      _radarSuspendedForRoute = false;
      _maybeKickVystrahyWarmupAfterRadar();
      return;
    }
    if (_radarContentReady &&
        !_isRadarLoading &&
        _radarMapUiReady &&
        !_radarSuspendedForRoute) {
      _maybeKickVystrahyWarmupAfterRadar();
      return;
    }
    setState(() {
  ...
  /// Karta radaru je v scrolli — WebView prejde z warmup hosta do panelu.
  void _noteRadarHomeCardVisible() {
    if (_radarHomeCardVisible) return;
    _radarHomeCardVisible = true;
    if (_radarContentReady || _radarLoadFailed) return;
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted || _radarContentReady || _radarLoadFailed) return;
      _nudgeRadarPlatformViewSize();
      unawaited(_injectRadarReadyGate());
    });
  }

  void _revealRadarMapLayers() {
    final ctrl = _radarController;
    if (ctrl == null) return;
    final city = currentCity ?? _lastRadarCity;
    if (city != null) {
...
'''));
      unawaited(ctrl.runJavaScript(buildMeteoRadarPinCityJs(
        lat: city.lat,
        lon: city.lon,
      )));
      return;
...
'''));
  ...
  /// Ak JS gate nestihne — po ~1.2 s force reveal.
  void _armRadarContentReadyFallback() {
    final armedGen = _radarLoadGeneration;
    _radarContentReadyTimer?.cancel();
    _radarContentReadyTimer = Timer(const Duration(milliseconds: 1200), () {
      _radarContentReadyTimer = null;
      if (!mounted ||
          armedGen != _radarLoadGeneration ||
          _radarContentReady ||
          _radarLoadFailed) {
        return;
      }
  ...
  /// Po načítaní HTML — záložný deadline (8 s).
  void _armRadarMapRevealDeadline() {
    final armedGen = _radarLoadGeneration;
    _radarLoadTimeoutTimer?.cancel();
    _radarLoadTimeoutTimer = Timer(const Duration(seconds: 8), () {
      _radarLoadTimeoutTimer = null;
      if (!mounted ||
          armedGen != _radarLoadGeneration ||
          _radarContentReady ||
      ...
      debugPrint('Radar: map reveal deadline — force ready');
      _markRadarContentReady();
    });
  }

  Future<bool> _radarMapLooksPainted({bool soft = false}) async {
    final ctrl = _radarController;
    if (ctrl == null) return false;
    try {
      // Soft = canvas + style; hard = + tiles. Bez sk-borders (Helkor ich často
      // nenačíta → starý check zasekol „Načítavam radar…“ navždy).
      final raw = await ctrl.runJavaScriptReturningResult(soft
          ? r'''
(function(){
  try {
...
(function(){
  try {
    ...
    if (typeof map.areTilesLoaded === 'function' && !map.areTilesLoaded()) {
      if (typeof map.loaded === 'function' && map.loaded()) return '1';
      return '0';
    }
    return '1';
  } catch (e) { return '0'; }
...
''');
  ...
  /// Prepni na live WebView — záložný paint check (fullscreen návrat).
  Future<void> _revealRadarLiveIfPainted() async {
    if (!mounted || _radarContentReady || _radarLoadFailed) return;
    for (var i = 0; i < 30; i++) {
      if (!mounted || _radarContentReady || _radarLoadFailed) return;
      if (await _radarMapLooksPainted()) {
        _markRadarContentReady();
        return;
      }
    ...
    for (var i = 0; i < 20; i++) {
      if (!mounted || _radarContentReady || _radarLoadFailed) return;
      if (await _radarMapLooksPainted(soft: true)) {
        _markRadarContentReady();
        return;
      }
      await Future<void>.delayed(const Duration(milliseconds: 50));
    }
    // Helkor/Mapbox niekedy nikdy nehlási tiles/borders — ukáž mapu aj tak.
    if (!mounted || _radarContentReady || _radarLoadFailed) return;
    debugPrint('Radar: force reveal po paint timeout');
    _markRadarContentReady();
  }

  /// CMAX na karte hneď; live WebView až po hraniciach + dlaždiciach (nie biela diera).
  ...
  try {
    ...
    if (!document.getElementById('app-radar-gate')) {
      ...
      s.textContent =
        ...
        // Mapa musí mať opacity 1 — Mapbox pri opacity:0 často nenačíta tiles.
        '#mapa,.mapboxgl-map{opacity:1!important;}' +
        'html:not(.radar-layers-ready) .mapboxgl-marker,' +
        'html:not(.radar-layers-ready) .mapboxgl-ctrl-bottom-left,' +
        'html:not(.radar-layers-ready) .mapboxgl-ctrl-logo{opacity:0!important;visibility:hidden!important;}';
    ...
    function bordersReady() {
      try {
        return !!(map.getSource && (
          map.getSource('sk-borders') ||
          map.getSource('radar-source-0') ||
          map.getSource('coverage-source')
        ));
      } catch (e) { return false; }
    }
    function radarLayersReady() {
      try {
    ...
    function tryFinish(tick) {
      if (done) return;
      if (!coreReady()) return;
      // Plné vrstvy = ideálne.
      if (radarLayersReady() && (tilesReady() || idleSeen)) {
        finish();
        return;
      }
      // Po ~1,2 s stačí canvas + style (bez sk-borders).
      if (tick >= 30 && (tilesReady() || idleSeen || bordersReady())) {
        finish();
        return;
      }
      // Po ~0,8 s force — inak Flutter spinner príliš dlho.
      if (tick >= 20) {
        finish();
      }
    }
    function bindMap() {
      try {
        ...
        map.on('idle', onIdle);
        if (typeof map.loaded === 'function' && map.loaded()) idleSeen = true;
        return true;
      } catch (e) { return false; }
    }
    bindMap();
...
''';
  ...
  String _radarRecenterJs(GeoCity city) => buildMeteoRadarPinCityJs(
        lat: city.lat,
        lon: city.lon,
        aggressiveRepin: true,
      );

  /// Návrat z fullscreen — pin pred aj po resize (setFullscreen(false) inak skočí na SR).
  String _radarReturnFromFullscreenJs(GeoCity city) => buildMeteoRadarPinCityJs(
        lat: city.lat,
        lon: city.lon,
        fullscreen: false,
  ...
  String _radarPrepareFullscreenJs(GeoCity city) => buildMeteoRadarPinCityJs(
        lat: city.lat,
        lon: city.lon,
        fullscreen: true,
        removeChrome: true,
  ...
  Future<void> _recenterRadarOnCity(GeoCity city) async {
    final ctrl = _radarController;
    if (ctrl == null) return;
    final int cameraSerial = ++_radarCameraSerial;
    try {
      final raw = await ctrl.runJavaScriptReturningResult(_radarRecenterJs(city));
      if (!mounted || cameraSerial != _radarCameraSerial) return;
      final ok = raw.toString().contains('1');
      if (!ok) {
        _setupRadarController(city, forceReload: true, cacheBust: true);
        return;
      }
      _lastRadarCity = city;
      _revealRadarMapLayers();
  ...
  /// true = hard reload URL; false = stačí jumpTo (mapa už beží).
  bool _radarShouldHardReloadOnCityChange({
    required bool cityChanged,
    required bool pendingForce,
  }) {
    if (_radarController == null) return true;
    // Helkor má userLat/mapCenter ako const z URL — soft pin nechá bodku na starom meste.
    if (cityChanged) return true;
    if (pendingForce) return true;
    return false;
  }

  /// Offstage / druhý host v strome zabíja Android PlatformView — len karta radaru.
  void _nudgeRadarPlatformViewSize() {
    final ctrl = _radarController;
    if (ctrl == null) return;
    WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  /// Pred fullscreen — rovnaká veľkosť ako full mapa, aby Mapbox stiahol dlaždice.
  Widget _buildRadarFullscreenPreloadHost() {
    if (!_radarFullscreenPreload || _radarWebViewWidget == null) {
  ...
  /// Počká na Mapbox dlaždice po resize (max ~1,5 s).
  Future<void> _waitRadarTilesLoaded(WebViewController ctrl) async {
    for (var i = 0; i < 30; i++) {
      if (!mounted || _radarController != ctrl) return;
      try {
...
(function(){
  try {
    ...
    return document.querySelector('.mapboxgl-canvas') ? '1' : '0';
  } catch (e) { return '0'; }
})()
''');
        final ok = raw.toString().contains('1');
  ...
  void _deferVystrahyWarmup(GeoCity city) {
    ...
    // HTTP banner hneď — WebView až po radare (2 PlatformView naraz sekajú emulátor).
    final preloader = VystrahyWebViewPreloader.instance;
    unawaited(preloader.refreshHomeBannerForCity(city));
    ...
    // Ak radar ešte loaduje, WebView výstrah počká na _maybeKickVystrahyWarmupAfterRadar.
    if (_supportsRadarForCity(city) &&
        !_radarContentReady &&
        !_radarLoadFailed) {
      return;
    }
    preloader.scheduleWarmup(delay: const Duration(milliseconds: 300));
  }
  ...
  Future<void> _configureRadarAndroidWebView(WebViewController ctrl) async {
    if (ctrl.platform is! AndroidWebViewController) return;
  ...
  Future<void> _openRadarFullscreen() async {
    ...
    // Náhľad sa NEZDIEĽA s full — vlastný WebView vo full (žiadny remount/biela diera).
    setState(() {
      _isRadarFullscreen = true;
      _radarFullscreenPreload = false;
      _radarFullscreenOpening = true;
    });
    ...
    await Navigator.of(context).push(
      PageRouteBuilder<void>(
        ...
        opaque: true,
        barrierColor: kAmbientBlendColor,
      ),
    );
    if (!mounted) return;
    await _restoreRadarHomeAfterFullscreen(city);
  }

  /// Rovnaký chrome ako FullscreenRadarPage — inak pri pushi preblikne hlavička.
  Widget _buildRadarFullscreenOpeningOverlay() {
    Widget chromeBtn(IconData icon, {EdgeInsetsGeometry? margin}) {
      return Container(
        width: 40,
        height: 40,
          ...
          Positioned(
            ...
            child: SafeArea(
              ...
              child: SizedBox(
                ...
                child: Row(
                  children: [
                    chromeBtn(Icons.arrow_back_rounded),
                    Expanded(
                      child: Center(
                        child: Container(
  ...
  /// Návrat z full — náhľadový controller ostal nedotknutý; len remount + reveal.
  Future<void> _restoreRadarHomeAfterFullscreen(GeoCity city) async {
    final ctrl = _radarController;
    if (ctrl != null) {
      _radarWebViewWidget = RepaintBoundary(
        ...
        child: buildMeteoRadarWebView(
          controller: ctrl,
          // Texture — Hybrid Composition na emulátore seká celú apku.
          hybridComposition: false,
        ),
      );
    }
    // Overlay zostáva — _isRadarReturning kryje kartu kým sa WebView neremountne.
    setState(() {
      _isRadarFullscreen = false;
    ...
    final homeCity = currentCity ?? city;
    _revealRadarMapLayers();
    try {
    ...
    // Krátky čas na repaint — potom hneď odkryj.
    await Future<void>.delayed(const Duration(milliseconds: 120));
    if (!mounted) return;

    // Overíme ci mapa žije; ak nie, tvrdý reload.
    final painted = await _radarMapLooksPainted(soft: true);
    if (!mounted) return;
    ...
    if (!painted) {
      ...
      _setupRadarController(homeCity, forceReload: true, cacheBust: true);
      return;
    }
  ...
  /// Generácia recenter/load — ignoruj zastarané jumpTo po rýchlom A→B→A.
  int _radarCameraSerial = 0;
  /// Len posledný hard reload v burstе skutočne volá loadRequest (inak 2–3× „Načítavam…“).
  int _radarLoadGeneration = 0;
  ...
  /// Domovská karta radaru už bola v strome — WebView prejde z warmup hosta do panelu.
  bool _radarHomeCardVisible = false;
  ...
  /// Pred push fullscreen — WebView v plnej veľkosti, Mapbox ťahá dlaždice.
  bool _radarFullscreenPreload = false;
  /// Overlay „Načítavam…“ až kým sa neotvorí fullscreen (bez bliku náhľadu).
  bool _radarFullscreenOpening = false;
  /// Search / iná route — skryť radar PlatformView (inak „dvojitý“ UI).
  bool _radarSuspendedForRoute = false;
  /// Nepriehľadný kryt počas odpojenia PlatformView — inak blikne prázdna karta radaru.
  final bool _routePlatformViewCover = false;
  ...
  void initState() {
    ...
    unawaited(_bindHomeWidgetLaunches());
  }

  Future<void> _bindHomeWidgetLaunches() async {
    try {
      final initial = await HomeWidget.initiallyLaunchedFromHomeWidget();
      if (mounted) _handleHomeWidgetLaunchUri(initial);
    } catch (e) {
      debugPrint('initiallyLaunchedFromHomeWidget: $e');
    }
    _homeWidgetClickSub?.cancel();
    _homeWidgetClickSub = HomeWidget.widgetClicked.listen(
      _handleHomeWidgetLaunchUri,
      onError: (Object e) => debugPrint('widgetClicked: $e'),
    );
  }

  void _handleHomeWidgetLaunchUri(Uri? uri) {
    if (!isVystrahyWidgetLaunchUri(uri)) return;
    _pendingOpenVystrahyFromWidget = true;
    unawaited(_tryOpenVystrahyFromWidget());
  }

  Future<void> _tryOpenVystrahyFromWidget() async {
    if (!_pendingOpenVystrahyFromWidget || _openingVystrahyFromWidget) return;
    if (!mounted) return;
    final city = currentCity;
    if (city == null || !_supportsVystrahyForCity(city)) {
      // Počasie / mesto ešte nie je hotové — skús znova po krátkom čakaní.
      Future<void>.delayed(const Duration(milliseconds: 450), () {
        if (mounted) unawaited(_tryOpenVystrahyFromWidget());
      });
      return;
    }
  ...
  Future<void> _appStartup() async {
    ...
    // Bez setState — inak sa najprv ukáže prázdny hero/radar a až potom celý obsah.
    if (widget.initialCity != null) {
      // Doplň ISO kód (WeatherAPI GPS často vráti prázdny countryCode).
      ...
      if (_supportsVystrahyForCity(bootCity)) {
        final preloader = VystrahyWebViewPreloader.instance;
        preloader.updateUserLocation(bootCity.lat, bootCity.lon, inject: false);
        preloader.prefetchAssets();
        if (!_supportsRadarForCity(bootCity)) {
          preloader.scheduleWarmup(delay: const Duration(milliseconds: 600));
        }
      }
    }
    await _loadSettings();
    ...
    if (widget.initialCity != null) {
      final bootCity = _normalizeCityCountryCode(widget.initialCity!);
      currentCity = bootCity;
      await fetchWeatherByCity(bootCity, forceRefresh: cachePurged);
      _updateOneSignalTags(bootCity);
    } else {
  ...
  void dispose() {
    ...
    _homeWidgetClickSub?.cancel();
    _homeWidgetClickSub = null;
    _heartbeatTimer?.cancel();
    _weatherPollTimer?.cancel();
    _radarWatchTimer?.cancel();
    _vystrahyBannerTimer?.cancel();
    _radarLoadTimeoutTimer?.cancel();
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
      ...
      final resumedCity = currentCity;
      if (resumedCity != null && _supportsVystrahyForCity(resumedCity)) {
        unawaited(
          VystrahyWebViewPreloader.instance.refreshHomeBannerForCity(resumedCity),
        );
      } else {
        VystrahyWebViewPreloader.instance.pruneExpiredActiveWarning(
          now: _getCurrentLocationTime(),
        );
      }
  ...
  void _setupRadarController(
    ...
    // Recenter len keď mapa naozaj beží — nie počas prvého loadu alebo po zlyhaní.
    if (_radarController != null &&
        preferRecenter &&
        hasCityChanged &&
        !forceReload &&
    ...
    if (_radarController == null || hasCityChanged || forceReload) {
      // Zruš bežiace recenter/jumpTo — inak starší A→B môže doraziť po B→A.
      _radarCameraSerial++;
      final int loadGen = ++_radarLoadGeneration;
      _lastRadarCity = city;
      ...
      if (_radarController == null) {
          ...
          ..setNavigationDelegate(
            NavigationDelegate(
              onPageStarted: (String url) {
                ...
                if (ctrl != null) {
                  unawaited(ctrl.runJavaScript(
                    'window.__radarReadyArmed=false;window.__radarReadyDone=false;',
                  ));
                }
                // Už sme v loade zo setupu — nepreklikávaj overlay znova setState.
                if (!_isRadarLoading || _radarLoadFailed) {
                  if (mounted) {
                    setState(() {
                      _isRadarLoading = true;
                      _radarLoadFailed = false;
              ...
              onPageFinished: (String url) {
                final finishedLoadGen = _radarLoadGeneration;
                _radarAutoRetryCount = 0;
                _radarOffline = false;
                _radarSuspendedForRoute = false;
                _armRadarMapRevealDeadline();
                _armRadarContentReadyFallback();
                unawaited(_injectRadarReadyGate());
                Future<void>.delayed(const Duration(milliseconds: 180), () {
                  if (!mounted ||
                      finishedLoadGen != _radarLoadGeneration ||
                      _radarContentReady ||
                      _radarLoadFailed) {
                    return;
                  }
                ...
                _nudgeRadarPlatformViewSize();
                // Pin podľa cieľa loadu (_lastRadarCity), nie currentCity —
                // pri zmene mesta môže currentCity ešte chvíľu zaostávať.
                final pinCity = _lastRadarCity ?? currentCity;
                if (mounted && pinCity != null) {
                  unawaited(_radarController!.runJavaScript(buildMeteoRadarPinCityJs(
                    lat: pinCity.lat,
                    lon: pinCity.lon,
                    fullscreen: _isRadarFullscreen ? true : null,
                    aggressiveRepin: true,
                  )));
                } else {
        ...
        _radarWebViewWidget = RepaintBoundary(
          key: _radarWebViewKey,
          // Texture mode — Hybrid Composition na Android emulátore zabíja FPS.
          // Biela diera v scrolle je krytá opaque loading overlayom.
          child: buildMeteoRadarWebView(
            controller: _radarController!,
            hybridComposition: false,
          ),
        );
        unawaited(_configureRadarAndroidWebView(_radarController!));
        _ensureRadarConnectivityWatcher();
      }

      final alreadyShowingLoad =
          _isRadarLoading && !_radarContentReady && !_radarLoadFailed;
      if (!alreadyShowingLoad) {
        if (mounted) {
          setState(() {
            _isRadarLoading = true;
            _radarLoadFailed = false;
      ...
      final ctrl = _radarController!;
      final loadCity = city;
      // Pri zmene mesta vždy cache-bust — inak WebView/Helkor nechá starý mapCenter.
      final loadCacheBust = cacheBust || hasCityChanged || forceReload;
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted || _radarController != ctrl) return;
        // Novší setup v tom istom frame — ten starší loadRequest zruš.
        if (loadGen != _radarLoadGeneration) return;
        // Ak medzičasom prišla novšia lokalita, nenačítavaj zastaranú URL.
        if (_lastRadarCity != null &&
            ((_lastRadarCity!.lat - loadCity.lat).abs() > 0.0001 ||
                (_lastRadarCity!.lon - loadCity.lon).abs() > 0.0001)) {
          return;
        }
        final loadUrl = buildMeteoRadarUrl(loadCity, cacheBust: loadCacheBust);
        unawaited(
          ctrl.runJavaScript('window.__radarReadyArmed = false;').catchError((_) {}),
        );
  ...
  void _handleRadarLoadFailure({required bool autoRetry}) {
    ...
    if (!autoRetry || _radarController == null || _lastRadarCity == null) {
      _maybeKickVystrahyWarmupAfterRadar();
      return;
    }
    if (_radarAutoRetryCount >= 2) {
      _maybeKickVystrahyWarmupAfterRadar();
      return;
    }
  ...
  void _restartPeriodicTimers() {
    ...
    // Výstrahy: vydanie / zrušenie / uplynutie bez manuálneho refreshu.
    _vystrahyBannerTimer?.cancel();
    _vystrahyBannerTimer = Timer.periodic(const Duration(seconds: 15), (_) {
      final city = currentCity;
      if (city == null || !_supportsVystrahyForCity(city)) return;
      final preloader = VystrahyWebViewPreloader.instance;
      preloader.pruneExpiredActiveWarning(now: _getCurrentLocationTime());
      unawaited(preloader.refreshHomeBannerForCity(city));
    });
  ...
  Widget _buildVystrahyTopBanner(VystrahyActiveNotice notice) {
    final now = _getCurrentLocationTime();
    final live = notice.prunedAt(now);
    if (live == null) return const SizedBox.shrink();

    final accent = live.accentColor;
    final items = live.visibleItemsAt(now);
    if (items.isEmpty) return const SizedBox.shrink();

    final single = items.length == 1;
    final item = single ? items.first : live.primaryItem!;
    final schedule = single
        ? item.timingLine(now)
        : live.multiScheduleLines(now);

    return Material(
      color: Colors.transparent,
      child: InkWell(
        borderRadius: BorderRadius.circular(18),
        onTap: () => unawaited(_openVystrahyPage()),
        child: Container(
          width: double.infinity,
          decoration: BoxDecoration(
            color: const Color(0xFF1A2A38).withValues(alpha: 0.96),
            borderRadius: BorderRadius.circular(18),
                ...
                child: Row(
                  ...
                  children: [
                    Container(
                      ...
                      alignment: Alignment.center,
                      decoration: BoxDecoration(
                        color: single ? item.accentColor : accent,
                        shape: BoxShape.circle,
                      ),
                      child: FaIcon(
                        item.icon,
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            single ? item.jav : live.countTitleSk(),
                            maxLines: 1,
                            overflow: TextOverflow.ellipsis,
                            style: const TextStyle(
                              color: Colors.white,
                          ...
                          Text(
                            single
                                ? item.levelLine(live.okres)
                                : live.multiLevelOkresLine(),
                            maxLines: 1,
                            overflow: TextOverflow.ellipsis,
                            style: TextStyle(
                              color: accent,
  ...
  Future<void> _openVystrahyPage({bool waitForReady = false}) async {
    ...
    // Z widgetu: počkaj na warmup. Z UI: hneď push (loading na stránke).
  ...
  /// Karta „Sledovač zrážok“ — active/incoming (a watching so signálom v okolí).
  RadarPrecipTrackerInfo? _radarPrecipTrackerInfoOrNull() {
    ...
    final stabilized = stabilizeRadarTrackerInfo(
      raw,
      locTime,
      cityLat: city.lat,
      cityLon: city.lon,
    );
    return ctx.finalizeTrackerCard(stabilized, locTime);
  }

  bool _shouldShowRadarPrecipTracker(RadarPrecipTrackerInfo info) {
    // Jasný stav + stredná istota (watching) pre možné zrážky v najbližších hodinách.
    if (info.phase != RadarPrecipTrackerPhase.active &&
        info.phase != RadarPrecipTrackerPhase.incoming &&
        info.phase != RadarPrecipTrackerPhase.watching) {
      return false;
    }
    // Po uplynutí endLocal kartu schovaj (12:55 pri 12:57).
    final end = info.endLocal;
    if (end != null) {
      final locTime = _getCurrentLocationTime();
      final endFloor = DateTime(
        end.year,
        end.month,
        end.day,
        end.hour,
      ...
      if (!endFloor.isAfter(nowFloor)) return false;
    }
    return true;
  }

  Widget _buildRadarPrecipTrackerSection(RadarPrecipTrackerInfo info) {
    // Pri príchode / aktívnom daždi zrážková ikona (nie hero „Zamračené“).
    final usePrecipIcon = info.phase == RadarPrecipTrackerPhase.active ||
        info.phase == RadarPrecipTrackerPhase.incoming ||
        info.phase == RadarPrecipTrackerPhase.watching;
    final heroCode = _getFirstHourWeatherInfo().$4;
    final statusCode = normalizeDisplayWeatherCode(
      usePrecipIcon ? info.iconCode : heroCode,
    );
    final icon = getWeatherIcon(
      statusCode,
      size: 52,
      daily: weatherData?.daily,
      hourTime: weatherData?.hourly?.time.isNotEmpty == true
          ? weatherData!.hourly!.time.first
          : null,
    );
    final rawStatus = usePrecipIcon
        ? weatherDescriptionSk(info.iconCode)
        : weatherDescriptionPinnedSk(statusCode);
    final statusLabel = (info.phase == RadarPrecipTrackerPhase.watching
            ? 'Možné zrážky'
            : rawStatus)
        .replaceFirstMapped(
  ...
  Widget? _buildHomeInsightTilesRow({
    Map<String, String>? uvWarning,
  }) {
  ...
  Future<void> _refreshRadarNowcastForCity(GeoCity city) async {
    ...
    try {
      ...
      // Nezmaž platný nowcast prázdnou odpoveďou.
      if (!ctx.eligible && _radarNowcastContext.eligible) return;
  ...
  String _buildDailySummaryBody(GeoCity city, WeatherData data) {
    ...
    final rainSegment = snowCodes.contains(todayCode) && snowfall >= 0.2
        ? 'Očakáva sa sneženie, napadnúť môže približne ${_formatPrecipitation(snowfall, weatherCode: todayCode, daily: true)}.'
        : precip > 0.2
            ? 'Očakávajú sa zrážky, ich úhrn môže dosiahnuť ${_formatPrecipitation(precip, daily: true)}.'
  ...
  Future<String?> _cachedWeatherJsonForCity(double lat, double lon) async {
    final wapi = await CacheManager.getWeather(
      lat,
      lon,
      weatherApiForecastCacheKey(lat, lon),
      ignoreExpiry: true,
    );
    if (wapi != null) return wapi;
  ...
  Future<void> _resolveCurrentLocation(GeoCity? fallbackCity) async {
    ...
    try {
      ...
      if (cityRaw != null) {
        ...
        // currentCity až vo fetchWeatherByCity — inak locationChanged=false a radar
        // ostane na seed meste / starom loadRequest.
        await fetchWeatherByCity(
          city,
          forceRefresh: false,
          showLoading: weatherData == null,
  ...
  Future<WeatherData> _fetchForecastWeatherData(
    ...
    Future<WeatherData?> fromCache({required bool allowStale}) async {
      final wapiJson = await CacheManager.getWeather(
        lat,
        lon,
        weatherApiForecastCacheKey(lat, lon),
        ignoreExpiry: allowStale,
      );
      WeatherData? data = await _weatherDataFromStoredJson(
        wapiJson,
        lat,
        lon,
        timezone,
      ...
      if (data == null) {
        ...
        data = await _weatherDataFromStoredJson(cachedJson, lat, lon, timezone);
      }
      if (data?.current != null) {
        try {
  ...
  Future<({WeatherData data, AirQualityData? aqi})?> _loadStoredWeatherForCity(
    GeoCity city, {
    ...
    try {
      ...
      // Satelit až po prvom painte — pri zmene lokality by to zdržalo cache.
      if (includeSatellite && data.current != null) {
        try {
          final satCloud = await fetchSatelliteCloudCover(city.lat, city.lon);
          if (satCloud != null) {
            final current = data.current!;
  ...
  /// Pri zmene mesta: radar/nowcast/výstrahy až PO prvom painte predpovede.
  void _kickLocationChangePipeline({
    required GeoCity city,
    required int mySerial,
    required int radarSerial,
    required bool wasOffline,
  }) {
    // Jeden hard reload — nie defer + setup naraz (to preblikávalo 2–3×).
    if (_supportsRadarForCity(city)) {
      prefetchRadarMapAssets(city);
      _pendingRadarForceReload = false;
      unawaited(_reloadRadarWithOptionalFreeze(city));
    } else {
      _deferRadarSetup(city, forceReload: true);
    }
    _kickRadarNowcastEarly(
      city: city,
      mySerial: mySerial,
  ...
  /// Hard reload — kryt „Načítavam…“; WebView ostáva v strome (CMAX + warmup).
  Future<void> _reloadRadarWithOptionalFreeze(GeoCity city) async {
    if (!_supportsRadarForCity(city)) return;
    if (!mounted) return;
    setState(() {
      _isRadarLoading = true;
  ...
  void _resetLocationTransientState() {
    _expandedStates = [];
  ...
  Future<void> fetchWeatherByCity(GeoCity city, {bool forceRefresh = false, bool showLoading = true}) async {
    if (!mounted) return;
    city = _normalizeCityCountryCode(city);
    ...
    // Výstrahy hneď pri zmene mesta — nenechať ich čakať na weather cache/API.
    if (locationChanged) {
      _syncVystrahyPreloaderForCity(city);
    }

    // Internet + cache naraz. Pri zmene mesta UI ešte NEMENÍME — inak vidno staré ikony pod novým názvom.
    final boot = await Future.wait<Object?>([
      hasInternetConnection(),
      _loadStoredWeatherForCity(city, includeSatellite: false),
    ]);
    if (!mounted || mySerial != _weatherFetchSerial) return;

    final bool hasInternet = boot[0] as bool;
    final stored = boot[1] as ({WeatherData data, AirQualityData? aqi})?;

    if (!hasInternet && !forceRefresh && stored != null) {
      ...
      setState(() {
        ...
        _isOffline = true;
        hasError = false;
        if (locationChanged) _resetLocationTransientState();
      });
      if (locationChanged) {
        _kickLocationChangePipeline(
          city: city,
          mySerial: mySerial,
          radarSerial: radarSerial,
      ...
      await _syncDailySummaryWithLatestData(city, stored.data);
      await _syncEveningSummaryWithLatestData(city, stored.data);
      await _syncAllLeadAlerts(stored.data);
      _scheduleWidgetUpdate();
      return;
    }

    final bool horizonTooShort = !forecastDailyHorizonComplete(
      locationChanged ? stored?.data : weatherData,
    );
    ...
    if (locationChanged) {
      // Jedno preklopenie: nové mesto + predpoveď naraz.
      // Bez cache nemeň UI — ostane predchádzajúca lokalita, kým nepríde API (žiadne „načítavanie ikoniek“).
      if (stored != null) {
        if (!mounted || mySerial != _weatherFetchSerial) return;
        setState(() {
          currentCity = city;
        ...
        // Bez cache: radar/výstrahy hneď na nové súradnice, ikony až s API.
        _kickLocationChangePipeline(
          city: city,
          mySerial: mySerial,
          radarSerial: radarSerial,
    ...
    try {
      ...
      // Ikony/predpoveď hneď — AQI neskôr (700 ms čakanie spomaľovalo prepínanie miest).
      setState(() {
        currentCity = city;
        weatherData = data;
        if (locationChanged && stored == null) {
          airQualityData = null;
        }
        isLoading = false;
        _isRefreshing = false;
      ...
      _invalidateDisplayCaches();
      // Radar už kickol _kickLocationChangePipeline na začiatku fetchu (bez cache).
      _scheduleSecondaryWeatherSignals(
        city: city,
        mySerial: mySerial,
        radarSerial: radarSerial,
      ...
      AirQualityData? aqiData;
      try {
        aqiData = await aqiFuture.timeout(const Duration(milliseconds: 700));
      } on TimeoutException {
        aqiData = null;
      } catch (e) {
        debugPrint('AQI fetch failed: $e');
      ...
      if (!mounted || mySerial != _weatherFetchSerial) return;
      if (aqiData != null) {
        setState(() => airQualityData = aqiData);
      }
    } catch (e) {
  ...
  GeoCity _normalizeCityCountryCode(GeoCity city) {
    final cc = city.countryCode.trim();
    if (cc.isNotEmpty) return city;
    final mapped = weatherApiCountryCodeFromName(city.country);
    if (mapped.isEmpty) return city;
    return GeoCity(
      name: city.name,
      lat: city.lat,
      lon: city.lon,
  ...
  Future<void> _onRefresh() async {
    ...
    try {
      final city = currentCity;
      final refreshTasks = <Future<void>>[_refreshCurrentDataOnly()];
      if (city != null && _supportsVystrahyForCity(city)) {
        refreshTasks.add(
          VystrahyWebViewPreloader.instance.refreshForHomePull(
            city.lat,
            city.lon,
            city: city,
          ),
      ...
      await syncVystrahyHomeWidgetFromNotice(
        VystrahyWebViewPreloader.instance.activeWarningNotice,
        fallbackOkres: city?.name,
        showMapHint: city != null && cityEligibleForVystrahy(city),
      );

      if (city != null) {
        await _fetchHistorical(city);
      }
    } catch (e) {
      if (mounted) {
        setState(() {
  ...
  void _scheduleWidgetUpdate() {
    ...
    Future<void>.microtask(() async {
      ...
      try {
        ...
        await syncVystrahyHomeWidgetFromNotice(
          VystrahyWebViewPreloader.instance.activeWarningNotice,
          fallbackOkres: currentCity?.name,
          showMapHint:
              currentCity != null && cityEligibleForVystrahy(currentCity!),
        );
      } catch (e) {
  ...
  Future<void> _openSearch() async {
    ...
    // Neodpájať radar pred pushom — inak mapa v náhľade zmizne.
    // Search je opaque (ColoredBox pod fade), Texture PlatformView ostane pod ním.
    final selected = await navigator.push<GeoCity>(
      PageRouteBuilder<GeoCity>(
        opaque: true,
        transitionDuration: const Duration(milliseconds: 160),
        reverseTransitionDuration: const Duration(milliseconds: 140),
        pageBuilder: (_, __, ___) => const CitySearchPage(),
    ...
    if (!mounted) return;
    // Po pop môže ešte dobehnúť IME — krátka pauza pred reloadom mesta.
    await Future<void>.delayed(const Duration(milliseconds: 120));
    if (!mounted) return;

    if (selected != null) {
      // Krátky kryt „Načítavam radar…“ — fetchWeatherByCity kickne jeden reload.
      if (_supportsRadarForCity(selected)) {
        setState(() {
          _radarSuspendedForRoute = true;
          _isRadarLoading = true;
          _radarContentReady = false;
        ...
        children: [
              ...
              if (_supportsVystrahyForCity(currentCity))
                ListenableBuilder(
                  ...
                  builder: (context, _) {
                    final now = _getCurrentLocationTime();
                    final raw = VystrahyWebViewPreloader
                        .instance.activeWarningNotice;
                    final notice = raw?.prunedAt(now);
                    if (raw != null && notice == null) {
                      WidgetsBinding.instance.addPostFrameCallback((_) {
                        VystrahyWebViewPreloader.instance
                            .pruneExpiredActiveWarning(now: now);
                      });
                      return const SizedBox.shrink();
                    }
                    if (notice == null || !notice.shouldShowAt(now)) {
                      return const SizedBox.shrink();
                    }
                    return Padding(
                      padding: const EdgeInsets.only(top: 10),
                      child: _buildVystrahyTopBanner(notice),
                    );
                  },
                ),
  ...
  void _openPollenPage() {
    final city = currentCity;
    if (city == null) return;
    final aqi = airQualityData ?? AirQualityData();
    ...
    unawaited(Navigator.of(context).push<void>(
      PageRouteBuilder<void>(
        ...
        reverseTransitionDuration: Duration.zero,
        opaque: true,
        pageBuilder: (_, __, ___) => PollenForecastPage(
          city: city,
          aqiData: aqi,
        ),
      ),
  ...
  void _openWeatherChart() {
    final city = currentCity;
    final data = weatherData;
    if (city == null || data == null) return;

    final radarCtx = radarNowcastActiveForCity(city) &&
            _radarNowcastContext.eligible
        ? _radarNowcastContext
    ...
    unawaited(Navigator.of(context).push<void>(
      PageRouteBuilder<void>(
        ...
        reverseTransitionDuration: Duration.zero,
        opaque: true,
        pageBuilder: (_, __, ___) => WeatherChartPage(
          city: city,
          data: data,
          radarCtx: radarCtx,
        ),
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!forecastDailyHorizonComplete(data)) {
        unawaited(
          fetchWeatherByCity(city, forceRefresh: false, showLoading: false),
        );
      }
    });
  }

  /// Radar karta: Helkor mapa (SK/CZ/…) a/alebo RainViewer sledovač (globálne).
  bool _shouldShowRadarHomeCard() {
    if (currentCity == null) return false;
    if (_supportsRadarForCity(currentCity)) return true;
    return radarNowcastActiveForCity(currentCity!);
  }

  Widget _buildRadarMapPanel() {
    final showLoadingOverlay = !_isRadarFullscreen &&
        !_isRadarReturning &&
        !_radarLoadFailed &&
        (!_radarContentReady || _isRadarLoading);
    final previewCity = currentCity ?? _lastRadarCity;

    return Container(
      key: _radarPanelHostKey,
      height: 220,
      decoration: appSurfaceDecoration(radius: 20),
      child: ClipRRect(
        ...
        child: Stack(
          alignment: Alignment.center,
          children: [
            // CMAX náhľad hneď (aj keď WebView ešte nie je ready).
            if (previewCity != null && !_radarLoadFailed && !_isRadarReturning)
              Positioned.fill(
                child: RadarHomeFastPreview(
                  lat: previewCity.lat,
                  lon: previewCity.lon,
                ),
              )
            ...
            if (showLoadingOverlay || _isRadarReturning)
              Positioned.fill(
  ...
  Widget _buildRadarCard() {
    ...
    final city = currentCity!;
    final showMap = _supportsRadarForCity(city);

    // Helkor WebView len v pokrytí mapy — sledovač ide aj mimo (Sofia, …).
    if (showMap) {
      if (!_radarMapUiReady || _radarController == null) {
    ...
    return Padding(
      ...
      child: Column(
        ...
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          if (showMap) _buildRadarMapPanel(),
        ],
      ),
    );
  }
          ...
          SliverToBoxAdapter(
            child: Container(
              ...
              padding: const EdgeInsets.fromLTRB(
                ...
                0,
                12,
                _kHomeForecastSectionGap,
              ),
              child: _buildHomeInsightTilesRow(uvWarning: uvWarning),
            ),
          ),
        if (_shouldShowRadarHomeCard())
          SliverToBoxAdapter(
              ...
              child: Row(
                ...
                children: [
                  InkWell(
                    onTap: () async {
                      try {
                        await openUrl(kWeatherApiAttributionUrl);
                      // ignore: empty_catches
                      } catch (e) {}
                    },
                    child: const Text(
                      'Predpoveď poskytuje WeatherAPI.com',
                      style: TextStyle(
  ...
  Widget _hourlyTile({
    ...
    if (!showRainPrecipColumn && _isClearStripSkyCode(iconCode)) {
      ...
      // Pipeline už nastavila 40 pred zrážkou — nerež na 30; vždy aspoň floor ikony.
      shownPct = math.max(rounded.clamp(10, 40), skyFloor);
    } else {
  ...
  Widget _dailyTile({
    ...
    final rainingAtPinNow = dayIndex == 0 &&
        (_radarNowcastContext.radarUiWetAtPin ||
    ...
    if (!rainingAtPinNow &&
      ...
      precipStr =
          '${_formatPrecipitation(0)}\nŠanca: ${_roundPrecipProbabilityForDisplay(footerProb)} %';
    } else if (displayMm >= kMeaningfulPrecipMmPerHour &&
        footerProb >= kMinPrecipProbPercent) {
      precipStr =
          '${_formatPrecipitation(displayMm, weatherCode: dailyMainIconCode, daily: true)}\nŠanca: ${_roundPrecipProbabilityForDisplay(footerProb)} %';
    } else if (kPrecipitationCodes.contains(dailyMainIconCode) &&
        footerProb >= kMinPrecipProbPercent) {
      precipStr =
          '${_formatPrecipitation(displayMm >= kMeaningfulPrecipMmPerHour ? displayMm : 0, weatherCode: dailyMainIconCode, daily: true)}\nŠanca: ${_roundPrecipProbabilityForDisplay(footerProb)} %';
    } else if (rainingAtPinNow) {
      precipStr =
          '${_formatPrecipitation(math.max(displayMm, 0.5), weatherCode: dailyMainIconCode, daily: true)}\nŠanca: ${_roundPrecipProbabilityForDisplay(math.max(footerProb, 60))} %';
    } else {
      precipStr =
      ...
      body: Stack(
        ...
        children: [
          ...
          if (_supportsRadarForCity(currentCity))
            _buildRadarFullscreenPreloadHost(),
          if (_radarFullscreenPreload || _radarFullscreenOpening)
            Positioned.fill(child: _buildRadarFullscreenOpeningOverlay()),
          // Skrytý WebView výstrah — až keď radar nie je v loade
          // (2× PlatformView naraz na Android emulátore zabíja FPS).
          if (_supportsVystrahyForCity(currentCity) &&
              !_radarFullscreenPreload &&
          ...
          // Kryt pred searchom — skryje odpojenie radar PlatformView.
          if (_routePlatformViewCover)
